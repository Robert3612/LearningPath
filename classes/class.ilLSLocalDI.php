...
class ilLSLocalDI extends Container
{
    public function init(
        ...
        $this["gui.learner"] = function ($c) use ($dic, $lsdic, $object) : ilObjLearningPathLearnerGUI {
            $has_items = count($c["learneritems"]->getItems()) > 0;
            $first_access = $c["learneritems"]->getFirstAccess();

            return new ilObjLearningPathLearnerGUI(
                $c["obj.ref_id"],
                $has_items,
                $first_access,
                $c["usr.id"],
        ...
        $this["gui.toc"] = function ($c) use ($dic) : ilLPTOCGUI {
            return new ilLPTOCGUI(
                $c["player.urlbuilder"],
                $dic["ilCtrl"]
            );
        };

        $this["gui.loc"] = function ($c) use ($dic) : ilLPLocatorGUI {
            return new ilLPLocatorGUI(
                $c["player.urlbuilder"],
                $dic["ui.factory"]
            );
        };
        ...
        $this["player.urlbuilder"] = function ($c) use ($dic, $data_factory) : LSUrlBuilder {
            $player_base_url = $dic['ilCtrl']->getLinkTargetByClass(
                'ilObjLearningPathLearnerGUI',
                \ilObjLearningPathLearnerGUI::CMD_VIEW,
                '',
                false,
                false
            );
        ...
        $this["player.kioskrenderer"] = function ($c) use ($dic) : ilKioskPageRenderer {
            $kiosk_template = new ilTemplate("tpl.kioskpage.html", true, true, 'Modules/LearningPath');
        ...
        $this["participants"] = function ($c) use ($dic) : ilLearningPathParticipants {
            return new ilLearningPathParticipants(
                $c["obj.obj_id"],
        ...
        $this["roles"] = function ($c) use ($dic, $current_user) : ilLearningPathRoles {
            return new ilLearningPathRoles(
                $c["obj.ref_id"],
                $c["obj.obj_id"],
                $c["participants"],
                $dic["ilCtrl"],
