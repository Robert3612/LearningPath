 ...
 */
class ilLearningPathImporter extends ilXmlImporter
{
    /**
    ...
    public function importXmlRepresentation($entity, $id, $xml, $mapping)
    {
        if ($new_id = $mapping->getMapping("Services/Container", "objs", $id)) {
            ...
            $this->obj = new ilObjLearningPath();
            $this->obj->create();
        }

        $parser = new ilLearningPathXMLParser($this->obj, $xml);
        $this->data = $parser->start();

        $mapping->addMapping("Modules/LearningPath", "lso", $id, $this->obj->getId());
    }
    ...
    protected function buildSettings(array $ls_settings)
    {
        ...
        if ($ls_settings["abstract_img"] != "") {
            $path = $this->getNewImagePath(ilLearningPathFilesystem::IMG_ABSTRACT, $ls_settings['abstract_img']);
        ...
        if ($ls_settings["extro_img"] != "") {
            $path = $this->getNewImagePath(ilLearningPathFilesystem::IMG_EXTRO, $ls_settings['extro_img']);
