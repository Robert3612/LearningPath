 ...
 */
class ilLearningPathMembershipMailNotification extends ilMailNotification
{
        ...
        switch ($this->getType()) {
            ...
            case self::TYPE_STATUS_CHANGED:
                foreach ($this->getRecipients() as $rcp) {
                    ...
                    $this->appendBody("\n\n");
                    $this->appendBody($this->createLearningPathStatus((int) $rcp));
                    $this->appendBody("\n\n");
                    $this->appendBody(
                        $this->getLanguageText('grp_mail_permanent_link')
                    );
    ...
    protected function createLearningPathStatus(int $usr_id) : string
    {
        $part = ilLearningPathParticipants::_getInstanceByObjId($this->getObjId());
