...
/**
 * Class ilObjLearningPathSettingsGUI
 */
class ilObjLearningPathSettingsGUI
{
    ...
    public function executeCommand()
    {
        ...
        switch ($cmd) {
            ...
            case self::CMD_CANCEL:
                ...
                break;
            default:
                throw new ilException("ilObjLearningPathSettingsGUI: " .
    ...
    private function initRTEInput(ilTextAreaInputGUI $inpt) : ilTextAreaInputGUI
    {
        ...
        //$inpt->setRTESupport($obj_id, "lso", "learningPath");
        return $inpt;
    }
    ...
    protected function update()
    {
        ...
        if ($inpt->getDeletionFlag()) {
            $settings = $settings->withDeletion(ilLearningPathFilesystem::IMG_ABSTRACT);
        } else {
            $img = $_POST[self::PROP_ABSTRACT_IMAGE];
            if ($img['size'] > 0) {
                $settings = $settings->withUpload($img, ilLearningPathFilesystem::IMG_ABSTRACT);
            }
        }

        $inpt = $form->getItemByPostVar(self::PROP_EXTRO_IMAGE);
        if ($inpt->getDeletionFlag()) {
            $settings = $settings->withDeletion(ilLearningPathFilesystem::IMG_EXTRO);
        } else {
            $img = $_POST[self::PROP_EXTRO_IMAGE];
            if ($img['size'] > 0) {
                $settings = $settings->withUpload($img, ilLearningPathFilesystem::IMG_EXTRO);
            }
        }
