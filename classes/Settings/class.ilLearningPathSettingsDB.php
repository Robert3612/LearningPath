 ...
 */
class ilLearningPathSettingsDB
{
    ...
    /**
     * @var ilLearningPathFilesystem
     */
    protected $ls_filesystem;

    public function __construct(ilDBInterface $database, ilLearningPathFilesystem $ls_filesystem)
    {
        $this->database = $database;
        $this->ls_filesystem = $ls_filesystem;
    }

    public function store(ilLearningPathSettings $settings)
    {
    ...
    public function delete(int $obj_id)
    {
        ...
        foreach ([ilLearningPathFilesystem::IMG_ABSTRACT, ilLearningPathFilesystem::IMG_EXTRO] as $pre) {
            $settings = $this->ls_filesystem->delete_image($pre, $settings);
        }
    ...
    public function getSettingsFor(int $lso_obj_id) : ilLearningPathSettings
    {
    ...
    ) : ilLearningPathSettings {
        return new ilLearningPathSettings(
            $obj_id,
            $abstract,
            $extro,
            $abstract_image,
    ...
    protected function insert(ilLearningPathSettings $settings)
    {
        $values = array(
            "obj_id" => array("integer", $settings->getObjId()),
            "abstract" => array("text", $settings->getAbstract()),
